...
- `AGENT_HOST` — Agent hostname or IP (e.g. `tritone-cards-agent` or `localhost`)
...
```bash
docker build -f agent/caddy/Dockerfile -t tritone-cards-caddy agent/caddy
```
...
docker run --rm -p 8030:8030 \
  -e CADDY_PORT=8030 \
  -e AGENT_HOST=tritone-cards-agent \
  -e AGENT_WS_PORT=8022 \
  -e AGENT_HTTP_PORT=8020 \
  tritone-cards-caddy
```

Replace `tritone-cards-agent` and ports with your agent host and ports. Caddy sends WebSocket requests (`Connection: Upgrade`) to `AGENT_HOST:AGENT_WS_PORT` (inbound transport) and other HTTP to `AGENT_HOST:AGENT_HTTP_PORT` (admin API).
